...
class AutomatedDataPipeline:
    ...
    async def fetch_dukascopy_data(self, start_date="2025-01-01", end_date=None):
        ...
        print("✅ dukascopy-node installed successfully")
        ...
        if result.returncode != 0:
            print(f"❌ Dukascopy fetch failed: {result.stderr}")
            return None
        ...
        if not csv_files:
            print("❌ No CSV file generated by dukascopy-node")
            return None

        latest_file = csv_files[0]
        print(f"✅ Data fetched successfully: {os.path.basename(latest_file)}")
