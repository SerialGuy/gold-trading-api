...
class AutomatedDataPipeline:
            ...
            async with httpx.AsyncClient(timeout=30) as client:
                ...
                if response.status_code == 200:
                    ...
                    if 'result' in data and data['result']:
                        ...
                        logger.info(f"Economic data fetched: {len(processed_econ)} records")
                        self.economic_data = processed_econ
                        return processed_econ
                    else:
                        logger.warning("No economic data available for the specified period")
                        return pd.DataFrame()
                else:
                    logger.warning(f"Economic data fetch failed: HTTP {response.status_code}")
                    return pd.DataFrame()
                ...
                try:
                    ...
                    try:
                        ...
                        if result.returncode != 0:
                            logger.error(f"Dukascopy fetch failed: {result.stderr}")
                            return None
                        ...
                        csv_files = [f for f in os.listdir('download/') if f.startswith('xauusd') and f.endswith('.csv')]
                        if not csv_files:
                            logger.error("No CSV file generated by dukascopy-node")
                            return None

                        latest_file = csv_files[0]
                        logger.info(f"Dukascopy data file found: {latest_file}")
                    ...
                    logger.error("Dukascopy fetch timed out")
                    return None
                except Exception as e:
                    logger.error(f"Error in Dukascopy fetch: {e}")
                    return None

            gold_df = await loop.run_in_executor(None, fetch_with_dukascopy)
            
            if gold_df is None:
                logger.warning("Dukascopy fetch failed, using fallback method...")
                return await self.fetch_fallback_gold_data(start_date, end_date)

            return await self.process_dukascopy_data(gold_df)

        except Exception as e:
            logger.error(f"Error fetching dukascopy data: {e}")
            return await self.fetch_fallback_gold_data(start_date, end_date)
    ...
    async def combine_datasets(self, gold_data, economic_data):
        ...
        if gold_data is None:
            logger.error("No gold data available for combination")
            return None
        ...
        try:
            ...
            if economic_data is not None and not economic_data.empty:
                ...
                if 'timestamp' in combined_df.columns and 'timestamp' in economic_data.columns:
                    ...
                    # Align timezone types to avoid merge errors
                    ...
                    logger.info("Data combined with economic indicators")
                else:
                    logger.warning("Could not merge economic data - timestamp column missing")
            else:
                logger.info("No economic data to combine")
